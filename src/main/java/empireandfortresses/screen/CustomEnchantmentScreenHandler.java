...
public class CustomEnchantmentScreenHandler extends ScreenHandler {
    ...
    public final int[] enchantmentMaterial = new int[] { 1, 1, 1 };
                    ...
                    for (int jx = 0; jx < 3; jx++) {
                        if (enchantmentLevelEntries != null && enchantmentLevelEntries[jx] != null) {
                            for (int k = 0; k < enchantments.size(); k++) {
                                Identifier enchantmentIdentifier = new Identifier(enchantments.getCompound(k).getString("id"));
    ...
    public boolean onButtonClick(PlayerEntity player, int id) {
        ...
        if (id >= 0 && id < 3) {
                ...
                this.context.run((world, pos) -> {
                    ...
                    if (enchantmentLevelEntry != null) {
                        ...
                        if (player instanceof ServerPlayerEntity serverPlayer) {
                            Criteria.ENCHANTED_ITEM.trigger(serverPlayer, itemStack3, this.enchantmentLevel[id]);
                        }
            ...
            this.context.run((world, pos) -> world.playSound(null, pos, SoundEvents.ITEM_BOOK_PAGE_TURN, SoundCategory.BLOCKS, 1.0F, world.random.nextFloat() * 0.1F + 0.9F));
